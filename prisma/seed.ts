...
async function main() {
  ...
  // 1. Clear existing database entries to ensure a clean slate
  ...
  const account = await prisma.instagramAccount.create({
    data: {
      userId: user.id,
      instagramId: '17841400000000001', // Standard Meta format
  ...
  // 5. Populate some historical activity logs for chart visual aesthetics
  const logsToCreate = [];
  const days = 7;
  
  // Create 15-20 random logs distributed over the last week
  const commenters = [
  ...
  // Insert all logs sequentially or using createMany
